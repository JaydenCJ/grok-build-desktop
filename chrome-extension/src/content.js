(() => {
    ...
    <div class="gd-shell">
      <div class="gd-badge" part="badge">
        ...
        <div class="gd-meta">
          ...
          <div class="gd-presence">visible assist</div>
        </div>
      </div>
  ...
  const presence = root.querySelector(".gd-presence");
  ...
  function setTabState(tabState) {
    ...
    presence.textContent = isControlling ? "focus guarded" : "read-only presence";
  ...
  function setAgentConfig(nextConfig = {}) {
    ...
    presence.textContent = agentConfig.focusGuard ? "focus guarded" : "manual override";
  }
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (message.type === "GROK_DESKTOP_AGENT_INPUT") {
      sendResponse(setInputValue(message));
      return false;
    }

    if (message.type === "GROK_DESKTOP_AGENT_DOM_ACTION") {
      sendResponse(domAction(message));
      return false;
    }
