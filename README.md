...
- **Non-blocking streaming UI (v0.2.0).** Grok runs use `--output-format streaming-json`. Events flow through a Rust queue, typed Tauri events, a `useSyncExternalStore` data store, and a Web Worker that renders markdown off the main thread. The composer, sidebar, settings panels, and theme toggle stay fully interactive while a run is streaming.
- **FIFO run queue (v0.2.0).** Type a new prompt while the previous one is still streaming and it joins the queue; the Composer button switches from `Send` to `Enqueue`. The queue is persisted in `~/Library/Application Support/Grok Desktop/runs.sqlite` and survives app restart. A startup banner asks whether to resume or cancel any pending tasks from the previous session. Finished runs auto-vacuum after 7 days.
- **Claude-Code-class status bar (v0.2.0).** Below the chat: `${elapsed}s · ≈${tokens} tokens · ${state}`. State text reflects the latest `streaming-json` event (`thinking…`, `writing…`, `done · EndTurn`).
