...
export function Sidebar({
  ...
  function openHistoryMenu(e: React.MouseEvent, item: HistoryPreview) {
    ...
    const groupSubmenu: ContextMenuItem[] = [
      { label: t('sidebar.menu.newGroup'), icon: <FolderPlus size={15} />, onClick: () => startNewGroup(id) },
      ...(groupNames.length ? [{ label: t('sidebar.menu.moveTo'), header: true } as ContextMenuItem] : []),
      ...groupNames.map((g) => ({
      ...
      ...(currentGroup
        ? [{ label: t('sidebar.menu.removeFromGroup'), separator: true, icon: <X size={15} />, onClick: () => setPromptGroupId(id, null) }]
        : []),
    ];
    ...
    const items: ContextMenuItem[] = [
      ...
      { label: t('sidebar.menu.saveToLibrary'), icon: <BookmarkPlus size={15} />, onClick: () => void savePromptToLibrary(id) },
      {
      ...
      { label: t('sidebar.menu.rename'), icon: <Pencil size={15} />, shortcut: "R", onClick: () => startRename(id) },
      { label: t('sidebar.menu.moveToGroup'), icon: <FolderInput size={15} />, shortcut: "G", submenu: groupSubmenu },
      {
      ...
      { label: t('sidebar.menu.deleteConversation'), icon: <Trash2 size={15} />, shortcut: "⌫", danger: true, separator: true, onClick: () => deleteSession(id) },
    ];
      ...
      return (
        <div className="history-rename" key={item.id}>
          <input
            ...
            placeholder={rowEdit.mode === "rename" ? t('sidebar.renamePrompt') : t('sidebar.newGroupName')}
            aria-label={rowEdit.mode === "rename" ? t('sidebar.renamePrompt') : t('sidebar.newGroupName')}
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.preventDefault();
      ...
      <aside className="app-sidebar">
            ...
            {primaryNavItems.map((item) => {
              ...
              return (
                <button
                  ...
                  onClick={handle}
                >
                  {item.id === "new-session" ? <Plus size={16} /> : item.id === "search" ? <Search size={16} /> : item.id === "tools" ? <Wrench size={16} /> : <Settings size={16} />}
              ...
              <>
                ...
                {historyView.archived.length > 0 ? (
                  <div className="history-group archived">
                    ...
                    >
                      <Archive size={12} /> {t('sidebar.archivedCount', { count: historyView.archived.length })}
        ...
        <section className="sidebar-health" aria-label={t('sidebar.toolHealth')}>
          ...
          <div className={`health-pill ${statusTone(grokToolStatus)}`}>
            <Zap size={15} />
            <span>{grokToolStatus?.installed ? t('sidebar.grokReady') : t('sidebar.grokMissing')}</span>
          </div>
